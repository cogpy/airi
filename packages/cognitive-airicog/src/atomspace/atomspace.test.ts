...
describe('atomSpace', () => {
  ...
  describe('patternMatch', () => {
    it('binds a variable to every atom that fills its position', () => {
      const dog = as.addNode('ConceptNode', 'Dog')
      const mammal = as.addNode('ConceptNode', 'Mammal')
      const pet = as.addNode('ConceptNode', 'Pet')
      as.addLink('InheritanceLink', [dog.id, mammal.id])
      as.addLink('InheritanceLink', [dog.id, pet.id])

      const results = as.patternMatch({
        kind: 'link',
        linkType: 'InheritanceLink',
        outgoing: [dog.id, '$parent'],
      })

      expect(results).toHaveLength(2)
      expect(results.map(result => result.bindings.get('parent')).sort()).toEqual(
        [mammal.id, pet.id].sort(),
      )
    })

    it('requires a repeated variable to bind the same atom in every position', () => {
      const a = as.addNode('ConceptNode', 'A')
      const b = as.addNode('ConceptNode', 'B')
      const selfLink = as.addLink('AssociativeLink', [a.id, a.id])
      as.addLink('AssociativeLink', [a.id, b.id])

      const results = as.patternMatch({
        kind: 'link',
        linkType: 'AssociativeLink',
        outgoing: ['$x', '$x'],
      })

      expect(results).toHaveLength(1)
      expect(results[0].atom.id).toBe(selfLink.id)
      expect(results[0].bindings.get('x')).toBe(a.id)
    })

    it('matches a fully literal outgoing set without binding anything', () => {
      const a = as.addNode('ConceptNode', 'A')
      const b = as.addNode('ConceptNode', 'B')
      const link = as.addLink('AssociativeLink', [a.id, b.id])
      ...
      expect(results).toHaveLength(1)
      expect(results[0].atom.id).toBe(link.id)
      expect(results[0].bindings.size).toBe(0)
    })

    it('rejects a pattern longer than the link it is matched against', () => {
      const a = as.addNode('ConceptNode', 'A')
      const b = as.addNode('ConceptNode', 'B')
      as.addLink('AssociativeLink', [a.id, b.id])
      ...
      const results = as.patternMatch({
        kind: 'link',
        linkType: 'AssociativeLink',
        outgoing: [a.id, b.id, '$third'],
      })

      expect(results).toHaveLength(0)
    })

    it('never matches a node when the pattern constrains outgoing', () => {
      const a = as.addNode('ConceptNode', 'A')

      const results = as.patternMatch({ outgoing: ['$anything'] })

      expect(results.some(result => result.atom.id === a.id)).toBe(false)
      expect(results).toHaveLength(0)
    })
  })
})
