...
export class AtomSpace {
  ...
  private matchesPattern(atom: Atom, pattern: AtomPattern): boolean {
    // Check kind
    if (pattern.kind && atom.kind !== pattern.kind)
      return false
    ...
    if (pattern.name && atom.kind === 'node') {
      if (!this.matchesWildcard(atom.name, pattern.name))
        return false
    }
    ...
    if (pattern.truthValue) {
      ...
      if (tv.minStrength !== undefined && atom.truthValue.strength < tv.minStrength)
        return false
      if (tv.maxStrength !== undefined && atom.truthValue.strength > tv.maxStrength)
        return false
    ...
    if (pattern.attentionValue) {
      ...
      if (av.minSti !== undefined && atom.attentionValue.sti < av.minSti)
        return false
      if (av.maxSti !== undefined && atom.attentionValue.sti > av.maxSti)
        return false
      if (av.minLti !== undefined && atom.attentionValue.lti < av.minLti)
        return false
      if (av.maxLti !== undefined && atom.attentionValue.lti > av.maxLti)
        return false
    }

    // Check outgoing
    if (pattern.outgoing && atom.kind === 'link') {
      if (!pattern.outgoing.every((id, i) => atom.outgoing[i] === id))
        return false
    }
  ...
  private matchesPatternWithBindings(
    ...
    // `outgoing` is the only field that admits variables, so every other
    // constraint is checked by the plain matcher, which compares IDs literally.
    const { outgoing, ...rest } = pattern

    if (!this.matchesPattern(atom, rest))
      return false
    if (!outgoing)
      return true
    if (atom.kind !== 'link')
    ...
    return this.matchesOutgoingWithBindings(atom.outgoing, outgoing, bindings)
  }

  private matchesOutgoingWithBindings(
    outgoing: string[],
    pattern: string[],
    bindings: VariableBindings,
  ): boolean {
    if (pattern.length > outgoing.length)
      return false
    ...
    for (let i = 0; i < pattern.length; i++) {
      ...
      // A variable repeated in one pattern must land on the same atom each time.
      if (bound !== undefined) {
        if (bound !== target)
          return false
        continue
