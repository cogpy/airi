...
export interface AtomPattern {
  ...
  /**
   * Match the link's outgoing set positionally: entry `i` is compared against
   * the link's `i`-th target, so a shorter pattern matches a longer link and a
   * longer pattern matches nothing.
   *
   * `query` compares every entry as a literal atom ID. `patternMatch` treats an
   * entry written as `$name` as a variable that binds to whatever atom sits at
   * that position, and requires a repeated `$name` to bind the same atom.
   */
  outgoing?: string[]
...
/**
 * Variable names (without the `$` sigil) bound to the atom IDs they matched.
 */
export type VariableBindings = Map<string, string>

/**
 * A single `patternMatch` hit: the matched atom and the variables it bound.
 */
