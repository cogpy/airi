...
export class PLN {
  ...
  deduction(
    ...
    if (!abLink || !bcLink)
      return null
    if (abLink.kind !== 'link' || bcLink.kind !== 'link')
      return null
    if (abLink.outgoing.length < 2 || bcLink.outgoing.length < 2)
      return null
  ...
  induction(
    ...
    if (!abLink || !acLink)
      return null
    if (abLink.kind !== 'link' || acLink.kind !== 'link')
      return null

    // Check if A matches
    if (abLink.outgoing[0] !== acLink.outgoing[0])
      return null
  ...
  abduction(
    ...
    if (!abLink || !cbLink)
      return null
    if (abLink.kind !== 'link' || cbLink.kind !== 'link')
      return null

    // Check if B matches
    if (abLink.outgoing[1] !== cbLink.outgoing[1])
      return null
  ...
  analogy(
    ...
    if (!abSimilarity || !acLink)
      return null
    if (abSimilarity.type !== 'SimilarityLink')
      return null
  ...
  private forwardChainRecursive(
    ...
    if (remainingDepth <= 0 || visited.has(atomId))
      return
    visited.add(atomId)
  ...
  private backwardChainRecursive(
    ...
    if (remainingDepth <= 0 || visited.has(goalId))
      return
    visited.add(goalId)
    ...
    if (goal.kind === 'link') {
      ...
      for (const abLink of potentialLinks) {
        if (abLink.outgoing[0] !== a)
          continue
        const b = abLink.outgoing[1]

        // Look for B->C
        for (const bcLink of potentialLinks) {
          if (bcLink.outgoing[0] !== b || bcLink.outgoing[1] !== c)
            continue
